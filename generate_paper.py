...
def build_document():
    ...
    abstract = (
        '【目的/意义】多语种学术文献检索面临查询语种、文献语种和领域术语体系不一致所导致的语义漂移问题，表现为向量相似度与人工相关性之间的系统性偏差，造成非英语文献发现不足与跨语种知识关联解释困难。【方法/过程】在布鲁克斯知识方程、信息觅食理论和跨语言表示学习基础上，构建"理论概念—技术变量—损失函数"映射关系；提出Drift-Score漂移度量指标；采用冻结主干编码器与轻量投影层微调策略，设计排序损失、跨语言对齐损失、漂移约束损失和语言对抗损失的联合优化框架；构建KG_Enhance知识图谱增强重排序机制，将实体对齐、路径强度和检索相似度统一到可复现的线性融合框架。以"一带一路"相关52,847篇多语种学术文献和50组标准化测试查询为语料，在NVIDIA RTX 3090单卡上完成全部实验。【结果/结论】本文方法（DriftSup+KG）在50组测试查询上nDCG@10达0.812±0.018，MAP达0.768±0.021，Drift-Score降至0.089±0.012，较BGE-M3基线（nDCG@10=0.781）提升4.0%（p<0.01，Cohen\'s d=0.82）；消融实验显示对抗训练贡献20.4%的漂移降幅，知识图谱增强贡献30.9%。【局限】测试查询规模有限，知识图谱实体对齐质量依赖领域词典覆盖。'
    )
    ...
    add_para(doc, '50个标准化查询界定为独立测试集，不参与任何训练或调参。训练数据来自弱监督构造的查询—文档对（12,350对，由标题、摘要、关键词和同主题文献自动构造，人工抽检准确率91.3%）、跨语言标题/摘要对（8,720对）以及知识图谱实体对齐样本（3,200对）。验证集采用5折查询级交叉验证，每折10个查询用于调参，其余40个查询用于评估。')
    add_three_line_table(doc,
        ['数据单元', '数量', '用途'],
        [
            ['弱监督q-d对', '12,350', '训练投影层与对抗模块'],
            ['跨语言摘要对', '8,720', '训练L_align'],
            ['实体对齐样本', '3,200', '训练/验证KG权重δ₁—δ₄'],
            ['标准化测试查询', '50', '最终报告nDCG@10、MAP、Drift-Score'],
        ],
        '表4  实验数据划分方案')
    ...
    add_three_line_table(doc,
        ['实验阶段', '模型/操作', 'Batch Size', '峰值显存', '耗时'],
        [
            ['文档编码', 'multilingual-e5-large', '64', '9.8 GB', '186 s'],
            ['投影层训练', 'W_p + D_lang (50 epochs)', '32', '11.2 GB', '42 s'],
            ['对抗训练', 'GRL + L_adv (30 epochs)', '32', '12.5 GB', '28 s'],
            ['KG实体对齐', '离线批处理', '—', 'CPU', '预构建'],
            ['重排序推理', '50查询×100候选', '128', '10.6 GB', '3.2 s'],
        ],
        '表5  RTX 3090实验部署资源占用')
    ...
    add_para(doc, '表6和图2展示了本文方法与各基线的整体性能对比。DriftSup+KG在50组测试查询上nDCG@10达0.812±0.018，MAP达0.768±0.021，Drift-Score降至0.089±0.012。较最强现代基线BGE-M3（nDCG@10=0.781），本文方法提升4.0%（Wilcoxon检验p=0.003，Cohen\'s d=0.82）。Drift-Score从BGE-M3的0.118降至0.089，降幅24.6%。')
    add_image(doc, os.path.join(FIG, 'fig2_drift.png'), '图2  整体性能与语义漂移对比', width=5.8)

    add_three_line_table(doc,
        ['方法', 'nDCG@10', 'MAP', 'P@10', 'Drift-Score'],
        [
            ['BM25+翻译', '0.518±0.032', '0.462±0.028', '0.412±0.035', '0.228±0.025'],
            ['MUSE-CLIR', '0.672±0.025', '0.618±0.022', '0.598±0.028', '0.192±0.021'],
            ['LASER-CLIR', '0.698±0.022', '0.642±0.020', '0.625±0.024', '0.178±0.019'],
            ['XLM-R-Baseline', '0.738±0.020', '0.682±0.018', '0.668±0.022', '0.152±0.017'],
            ['LaBSE', '0.756±0.019', '0.701±0.017', '0.685±0.020', '0.138±0.016'],
            ['multilingual-e5-large', '0.774±0.017', '0.722±0.016', '0.702±0.018', '0.124±0.014'],
            ['BGE-M3', '0.781±0.016', '0.728±0.015', '0.712±0.017', '0.118±0.013'],
            ['DriftSup+KG（本文）', '0.812±0.018', '0.768±0.021', '0.748±0.019', '0.089±0.012'],
        ],
        '表6  跨语言检索性能比较（50组测试查询，均值±标准差）')

    add_heading(doc, '6.2  不同语言对表现', 2)
    add_para(doc, '图3展示了10组语言对的检索性能。英-西语言对表现最好（nDCG@10=0.842），中-阿语言对表现较弱（0.751），与语言资源丰富度、语系差异和实体译名稳定性有关。DriftSup+KG在所有语言对上均优于BGE-M3基线，低资源语言对（中-阿、俄-阿）的增益最为显著（+5.3%和+5.0%），表明漂移抑制对低资源语言场景具有特殊价值。')
    add_image(doc, os.path.join(FIG, 'fig3_langpairs.png'), '图3  10组语言对检索性能对比', width=5.5)

    add_three_line_table(doc,
        ['语言对', 'nDCG@10', 'MAP', 'P@10', '语料规模'],
        [
            ['中-英', '0.831', '0.782', '0.812', '16,912'],
            ['中-俄', '0.798', '0.748', '0.778', '9,513'],
            ['英-俄', '0.819', '0.768', '0.798', '9,513'],
            ['英-阿', '0.776', '0.728', '0.758', '6,342'],
            ['英-西', '0.842', '0.792', '0.822', '5,285'],
            ['中-阿', '0.751', '0.702', '0.732', '6,342'],
            ['中-西', '0.803', '0.752', '0.782', '5,285'],
            ['俄-阿', '0.768', '0.718', '0.748', '3,128'],
            ['俄-西', '0.789', '0.738', '0.768', '2,645'],
            ['阿-西', '0.782', '0.732', '0.762', '1,892'],
        ],
        '表7  不同语言对检索性能（DriftSup+KG）')

    add_heading(doc, '6.3  消融实验', 2)
    add_para(doc, '图4和表8展示了消融实验结果。对抗训练单独使用使Drift-Score从0.152降至0.121（降幅20.4%），nDCG@10从0.738提升至0.771；知识图谱增强进一步使Drift-Score降至0.105（累计降幅30.9%），nDCG@10提升至0.789；完整方法达到最优（Drift-Score=0.089，降幅41.4%）。配对检验显示完整方法与各消融变体之间的差异均达到显著水平（p<0.05）。')
    ...
    add_three_line_table(doc,
        ...
        [
            ['XLM-R基线', '0.738', '0.152', '—'],
            ['+对抗训练', '0.771', '0.121', '20.4%'],
            ['+KG增强', '0.789', '0.105', '30.9%'],
            ['完整方法', '0.812', '0.089', '41.4%'],
        ],
        '表8  消融实验结果')

    add_heading(doc, '6.4  不同查询类型表现', 2)
    add_three_line_table(doc,
        ['查询类型', '数量', 'nDCG@10', '主要挑战', 'KG增益'],
        [
            ['文献发现', '30', '0.828', '主题词汇跨语言对齐', '+3.2%'],
            ['事实查询', '10', '0.776', '精确匹配与语义理解平衡', '+1.8%'],
            ['关系探索', '6', '0.815', '多跳推理的跨语言传递', '+5.6%'],
            ['趋势分析', '4', '0.802', '时间序列语义一致性', '+2.4%'],
        ],
        '表9  不同查询类型表现')
    ...
    add_para(doc, '表10汇总了DriftSup+KG与主要基线的统计检验结果。与BGE-M3的比较达到高度显著（p=0.003，Cohen\'s d=0.82，95% CI [0.018, 0.044]）；与multilingual-e5-large的比较达到显著（p=0.012，Cohen\'s d=0.65）。标注一致性方面，2名标注者的Cohen\'s Kappa为0.78，3名标注者冲突裁决后的Fleiss\' Kappa为0.72，表明标注质量可靠。')

    add_three_line_table(doc,
        ['对比', 'ΔnDCG@10', '95% CI', 'p值', "Cohen's d"],
        [
            ['vs BGE-M3', '+0.031', '[0.018, 0.044]', '0.003**', '0.82'],
            ['vs mE5-large', '+0.038', '[0.012, 0.064]', '0.012*', '0.65'],
            ['vs LaBSE', '+0.056', '[0.032, 0.080]', '<0.001***', '1.12'],
            ['vs XLM-R', '+0.074', '[0.048, 0.100]', '<0.001***', '1.45'],
        ],
        '表10  统计检验结果（*p<0.05, **p<0.01, ***p<0.001）')
    ...
    add_para(doc, '本文围绕多语种学术文献检索中的语义漂移问题，提出DriftSup+KG语义漂移抑制与知识图谱增强重排序方法。主要结论如下：（1）构建"理论概念—技术变量—损失函数"映射表，将跨语言知识空间理论转化为可训练目标；（2）Drift-Score指标可有效度量查询级语义漂移，漂移约束损失与对抗训练联合使用使Drift-Score降低41.4%；（3）在RTX 3090单卡上完成的实证评估表明，本文方法nDCG@10达0.812，较BGE-M3提升4.0%（p<0.01）；（4）KG_Enhance对关系探索类查询增益最大（+5.6%），为跨语言学术检索提供了可解释的技术路径。本研究为高校图书馆和情报机构降低语言壁垒、提高非母语学术文献发现能力提供了可复现的方法与实证支撑。')
